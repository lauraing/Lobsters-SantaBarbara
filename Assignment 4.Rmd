...
lob_size_legal <- mutate(lob_size, LEGAL = SIZE>=82.6) %>% 
  filter(LEGAL == TRUE) %>% 
  count(SITE, LEGAL) %>% 
  spread(LEGAL, n)
  
lob_size_illegal <- mutate(lob_size, ILLEGAL = SIZE>=82.6) %>% 
  filter(ILLEGAL == FALSE) %>% 
  count(SITE, ILLEGAL) %>% 
  spread(ILLEGAL, n)

# how to combine the above data frames into one table??
